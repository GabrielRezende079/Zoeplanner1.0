...
import { supabase, type DbTransaction, type DbTithing, type DbGoal, type DbExpense } from '../lib/supabase';
...
export const useFinance = create<FinanceState>((set, get) => ({
  ...
  loadUserData: async () => {
    ...
    try {
      ...
      const [
        ...
        supabase
          .from('transactions')
          .select('id, user_id, type, amount, description, category, date, payment_type, destination_bank_id, created_at')
          .eq('user_id', user.id)
      ...
      if (tithingsError) console.error('❌ Erro ao carregar dízimos:', tithingsError);
  ...
  addTransaction: async (transaction) => {
    ...
    if (transaction.destination_bank_id) {
      try {
        ...
        const bankBalances = banksStore.getBankBalances(transaction.destination_bank_id);
        
        // Pegar o saldo mais recente do banco
        let currentBalance = 0;
        if (bankBalances.length > 0) {
          const latestBalance = bankBalances.sort((a, b) => 
        ...
        await banksStore.addAccountBalance({
          ...
          balance: newBalance,
          date: transaction.date,
          notes: `Atualização automática - ${transaction.type === 'income' ? 'Receita' : 'Despesa'}: ${transaction.description}`
        });
        
        console.log('✅ Saldo do banco atualizado automaticamente');
        
      } catch (balanceError) {
        console.error('⚠️ Erro ao atualizar saldo do banco (transação foi salva):', balanceError);
        // Não falha a transação se houver erro no saldo
      }
    } else {
      console.log('ℹ️ Transação sem banco específico - contabilizada no saldo geral');
    }
  ...
  removeTransaction: async (id) => {
    ...
    if (transactionToDelete?.destination_bank_id) {
      try {
        const banksStore = useBanks.getState();
        const bankBalances = banksStore.getBankBalances(transactionToDelete.destination_bank_id);
        ...
        await banksStore.addAccountBalance({
          ...
          notes: `Reversão automática - ${transactionToDelete.type === 'income' ? 'Receita' : 'Despesa'} removida: ${transactionToDelete.description}`
        });
        
        console.log('✅ Saldo do banco revertido automaticamente');
        
      } catch (balanceError) {
        console.error('⚠️ Erro ao reverter saldo do banco (transação foi removida):', balanceError);
        // Não falha a remoção se houver erro na reversão do saldo
      }
    } else {
      console.log('ℹ️ Transação sem banco específico removida - saldo geral atualizado automaticamente');
    }
  ...
  getNetBalance: () => {
    ...
    const transactionsWithoutBank = state.transactions.filter(t => !t.destination_bank_id);
    const generalBalance = transactionsWithoutBank.reduce((sum, transaction) => {
