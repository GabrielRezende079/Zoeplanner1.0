// Função utilitária para URL de redefinição de senha
const getResetPasswordUrl = () => window.location.origin + "/reset-password";
...
export const useAuth = create<AuthState>((set, get) => ({
  ...
  signup: async (userData) => {
    ...
    // Não insere na tabela users aqui!
  },

  // Novo método: criar perfil do usuário após login/autenticação
  createUserProfile: async (profileData: {
    ...
    const newUser = {
      ...
      tithing_practice: profileData.tithingPractice,
  ...
  updatePassword: async (newPassword: string) => {
    try {
      ...
      if (sessionError) {
        ...
        throw new Error(
          "Sessão inválida. Solicite um novo link de recuperação."
        );
      }

      if (!session) {
        throw new Error(
          "Nenhuma sessão ativa encontrada. Solicite um novo link de recuperação."
        );
      }
