...
const Banks: React.FC = () => {
  ...
  const [activeForms, setActiveForms] = useState<{
    [bankId: string]: "balance" | "investment" | "card" | null;
  }>({});
  ...
  const handleInvestmentSubmit = async (e: React.FormEvent, bankId: string) => {
    ...
    try {
      await addInvestment({
        ...
        final_value: investmentFormData.final_value
          ? parseFloat(investmentFormData.final_value)
          : undefined,
  ...
  const handleDeleteClick = (
    type: "bank" | "balance" | "investment" | "card",
    item: any,
    bankId?: string
  ) => {
    let itemName = "";
    ...
    switch (type) {
      case "bank":
        ...
        break;
      case "balance":
        itemName = `Saldo de R$ ${item.balance.toLocaleString(
          "pt-BR"
        )} em ${item.date.split("-").reverse().join("/")}`;
        break;
      case "investment":
        itemName = `${item.type} - R$ ${item.initial_value.toLocaleString(
          "pt-BR"
        )}`;
        break;
      case "card":
        itemName = `Cartão ${
          item.type === "debito" ? "de Débito" : "de Crédito"
  ...
  const setActiveForm = (
    bankId: string,
    formType: "balance" | "investment" | "card" | null
  ) => {
  ...
  const totalBalance = banks.reduce(
    (sum, bank) => sum + getLatestBalance(bank.id),
    0
  );

  // Calculate total investments across all banks
  const totalInvestments = banks.reduce(
    (sum, bank) => sum + getTotalInvestments(bank.id),
    0
  );
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="relative overflow-hidden bg-gradient-to-r from-blue-50 via-olive-50 to-gold-50 rounded-2xl p-6 border border-blue-200 shadow-lg mb-2">
        <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-blue-400 via-olive-500 to-gold-500"></div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <p className="text-gray-700">
                Gerencie seus bancos, saldos e investimentos
              </p>
            </div>
            <button
              onClick={() => setShowBankForm(true)}
              className="btn btn-primary shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-200 w-full sm:w-auto"
            >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        <div className="card p-6 bg-gradient-to-br from-green-50 via-white to-olive-50 border-2 border-green-200 shadow-xl">
        ...
        <div className="card p-6 bg-gradient-to-br from-blue-50 via-white to-olive-50 border-2 border-blue-200 shadow-xl">
          <div className="flex items-center mb-3">
        ...
        <div className="card p-6 bg-gradient-to-br from-gold-50 via-white to-olive-50 border-2 border-gold-200 shadow-xl sm:col-span-2 lg:col-span-1">
          <div className="flex items-center mb-3">
        ...
        {banks.length > 0 ? (
                  ...
                  <div className="border-t bg-gray-50">
                    ...
                    <div className="p-4 grid grid-cols-2 sm:grid-cols-4 gap-3">
                      ...
                      <div className="bg-white p-3 rounded-lg shadow-sm text-center">
                        ...
                        <p className="text-xs font-semibold">
                          R$ {totalInvestmentsValue.toLocaleString("pt-BR")}
                        </p>
                      </div>
                    </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                        <button
                          onClick={() =>
                            setActiveForm(
                              bank.id,
                              activeForm === "balance" ? null : "balance"
                            )
                          }
                        ...
                        <button
                          onClick={() =>
                            setActiveForm(
                              bank.id,
                              activeForm === "investment" ? null : "investment"
                            )
                          }
                          className={`btn w-full text-sm ${
                    ...
                    {activeForm === "investment" && (
                      <div className="border-t bg-white p-4">
                        ...
                        <form
                          onSubmit={(e) => handleInvestmentSubmit(e, bank.id)}
                        >
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
                            <div>
                              <label className="block text-sm font-medium text-gray-700 mb-1">
                            ...
                            <div className="space-y-2">
                              {bankCards.map((card) => (
                                ...
                                >
                                  ...
                                  <button
                                    onClick={() =>
                                      handleDeleteClick("card", card, bank.id)
                                    }
                                    className="text-red-600 hover:text-red-800 p-1 rounded transition-colors duration-200"
                                    title="Excluir"
                                  >
          ...
          <div className="card p-6 sm:p-8 text-center">
            ...
            <p className="text-gray-600 mb-4 text-sm sm:text-base">
              Cadastre seus bancos para ter controle completo de suas contas,
