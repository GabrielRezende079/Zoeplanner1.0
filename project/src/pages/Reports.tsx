...
import { FileText, Download, BarChart2, Heart, TrendingUp, TrendingDown, Calendar, CheckCircle, Target, AlertTriangle, Lightbulb, ArrowRight } from 'lucide-react';
import { 
  BarChart, 
  Bar, 
  XAxis, 
...
const Reports: React.FC = () => {
  ...
  const monthlyPaidExpenses = filteredExpenses.reduce((sum, e) => sum + e.amount, 0);
  ...
  const monthlyNetBalance = monthlyIncome - monthlyExpenses - monthlyPaidExpenses - monthlyGiving;
  
  // Tithe percentage calculation
  const tithePercentage = monthlyIncome > 0 
    ? (monthlyTithes / monthlyIncome) * 100 
    : 0;
  ...
  const expenseCategoriesFromExpenses = filteredExpenses
    .reduce((acc, expense) => {
  ...
  const expenseCategories = combinedExpenseCategories.sort((a, b) => b.value - a.value);
  
  // Income vs Expense data for bar chart - ROUNDED VALUES
  const incomeVsExpenseData = [
    { name: 'Receitas', value: Math.round(monthlyIncome * 100) / 100 },
    { name: 'Despesas Transações', value: Math.round(monthlyExpenses * 100) / 100 },
    { name: 'Despesas Pagas', value: Math.round(monthlyPaidExpenses * 100) / 100 },
  ...
  const monthOptions = availableMonths.map(month => {
    ...
    const hasTithingRecords = tithingRecords.some(r => r.date.startsWith(month));
  ...
  return (
    <div className="space-y-6">
          ...
          {showSuccessMessage && (
            <div className="absolute top-full right-0 mt-2 bg-green-50 border border-green-200 rounded-lg p-3 shadow-lg z-10 min-w-64">
              <div className="flex items-center">
                ...
                <div>
                  <p className="text-sm font-medium text-green-800">PDF gerado com sucesso!</p>
                  <p className="text-xs text-green-600">O download foi iniciado automaticamente.</p>
                </div>
              </div>
            </div>
          )}
      ...
      <div className="card p-4 relative overflow-visible">
        ...
        <div className="max-w-md relative">
          ...
          <div className="mt-2 text-xs text-gray-500 space-y-1">
            <p>📊 Mostrando dados de <span className="font-medium">{formatMonthName(selectedMonth)}</span></p>
            <p>💡 Apenas meses com movimentação financeira ou o mês atual são exibidos</p>
            {availableMonths.length === 1 && (
              <p className="text-amber-600">⚠️ Apenas o mês atual está disponível - adicione transações para ver mais opções</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="card p-4">
        <h2 className="section-title">Resumo Mensal: {formatMonthName(selectedMonth)}</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
          <div className="stats-card">
            <div className="flex items-center mb-2">
              ...
              <span className="ml-2 text-sm font-medium text-gray-600">Receitas</span>
            </div>
            <p className="text-xl font-semibold text-gray-800">R$ {monthlyIncome.toLocaleString('pt-BR')}</p>
          </div>
          ...
          <div className="stats-card">
            <div className="flex items-center mb-2">
              ...
              <span className="ml-2 text-sm font-medium text-gray-600">Despesas Transações</span>
            </div>
            <p className="text-xl font-semibold text-gray-800">R$ {monthlyExpenses.toLocaleString('pt-BR')}</p>
          </div>
          ...
          <div className="stats-card">
            <div className="flex items-center mb-2">
              ...
              <span className="ml-2 text-sm font-medium text-gray-600">Despesas Pagas</span>
            </div>
            <p className="text-xl font-semibold text-gray-800">R$ {monthlyPaidExpenses.toLocaleString('pt-BR')}</p>
          </div>
          ...
          <div className="stats-card">
            <div className="flex items-center mb-2">
              ...
              <span className="ml-2 text-sm font-medium text-gray-600">Dízimos/Ofertas</span>
            </div>
            <p className="text-xl font-semibold text-gray-800">R$ {monthlyGiving.toLocaleString('pt-BR')}</p>
          </div>
          
          <div className="stats-card">
            <div className="flex items-center mb-2">
              <div className={`h-8 w-8 rounded-full ${monthlyBalance >= 0 ? 'bg-blue-100' : 'bg-red-100'} flex items-center justify-center shadow-sm`}>
                <BarChart2 className={`h-4 w-4 ${monthlyBalance >= 0 ? 'text-blue-600' : 'text-red-600'}`} />
              </div>
              <span className="ml-2 text-sm font-medium text-gray-600">Saldo Bruto</span>
            </div>
            <p className={`text-xl font-semibold ${monthlyBalance >= 0 ? 'text-blue-600' : 'text-red-600'}`}>
          ...
          <div className="stats-card">
            <div className="flex items-center mb-2">
              <div className={`h-8 w-8 rounded-full ${monthlyNetBalance >= 0 ? 'bg-green-100' : 'bg-red-100'} flex items-center justify-center shadow-sm`}>
                <BarChart2 className={`h-4 w-4 ${monthlyNetBalance >= 0 ? 'text-green-600' : 'text-red-600'}`} />
              </div>
              <span className="ml-2 text-sm font-medium text-gray-600">Saldo Líquido</span>
            </div>
            <p className={`text-xl font-semibold ${monthlyNetBalance >= 0 ? 'text-green-600' : 'text-red-600'}`}>
              R$ {monthlyNetBalance.toLocaleString('pt-BR')}
            </p>
            <p className="text-xs text-gray-500 mt-1">
              (Após dízimos/ofertas)
            </p>
          </div>
        </div>
          ...
          <div className="explanation-banner">
            <div className="flex items-start">
              ...
              <p className="text-sm text-gold-700">
                <span className="font-medium">Impacto da Fidelidade:</span> Seus dízimos e ofertas 
                (R$ {monthlyGiving.toLocaleString('pt-BR')}) foram descontados do saldo bruto, 
                resultando no saldo líquido disponível.
              </p>
            </div>
          </div>
        )}
        ...
        <div className="mb-6">
          ...
          <div className="progress-bar">
            <div 
              className={`progress-fill ${tithePercentage >= 10 ? 'bg-green-600' : 'bg-olive-600'}`}
              style={{ width: `${Math.min(tithePercentage * 10, 100)}%` }}
            ></div>
          </div>
          <div className="flex justify-between text-xs text-gray-500">
            <span>0%</span>
            <span className={tithePercentage >= 10 ? 'text-green-600 font-medium' : ''}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="chart-container">
            <h3 className="text-md font-medium mb-3">Receitas vs Despesas vs Dízimos/Ofertas</h3>
            <div className="h-64">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="stats-card">
              ...
              <div className="scripture">
                "Porque a raiz de todos os males é o amor ao dinheiro..." (1 Timóteo 6:10)
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="relative bg-white border-4 border-azure-300 rounded-2xl shadow-2xl p-8 hover:shadow-3xl transition-shadow duration-300">
          ...
          <div className="flex items-center mb-6">
            ...
            <div>
              <h2 className="text-2xl font-bold text-gray-900 mb-1">Passos para o Próximo Mês</h2>
              <p className="text-azure-600 font-medium">Seu plano de ação personalizado baseado na análise financeira</p>
            </div>
          </div>
          ...
          <div className="flex items-center mb-6 bg-gradient-to-r from-red-50 to-orange-50 border-l-4 border-orange-400 p-4 rounded-r-lg">
            ...
            <div>
              ...
              <p className="text-sm text-orange-700">Implementar estas recomendações pode melhorar significativamente sua saúde financeira</p>
            </div>
          </div>
              ...
              <div className="group bg-gradient-to-r from-red-50 to-red-100 border-2 border-red-200 rounded-xl p-6 hover:shadow-lg transition-all duration-300">
                <div className="flex items-start">
                  ...
                  <div className="flex-1">
                    <h3 className="font-bold text-red-800 text-lg mb-2">URGENTE: Equilibrar Orçamento</h3>
                    <p className="text-red-700 mb-3">
                      Revise seus gastos e identifique áreas para redução de despesas, mantendo a fidelidade nos dízimos.
                    </p>
            ...
            {tithePercentage < 10 && (
              <div className="group bg-gradient-to-r from-gold-50 to-gold-100 border-2 border-gold-200 rounded-xl p-6 hover:shadow-lg transition-all duration-300">
                <div className="flex items-start">
                  ...
                  <div className="flex-1">
                    <h3 className="font-bold text-gold-800 text-lg mb-2">Ajustar Fidelidade nos Dízimos</h3>
              ...
              <div className="flex items-start">
                <div className="h-10 w-10 rounded-full bg-olive-500 flex items-center justify-center mr-4 shadow-lg group-hover:scale-110 transition-transform duration-300">
                  <span className="text-white text-lg font-bold">
                    {(monthlyNetBalance < 0 && tithePercentage < 10) ? '3' : 
                     (monthlyNetBalance < 0 || tithePercentage < 10) ? '2' : '1'}
                  </span>
                </div>
                <div className="flex-1">
                  <h3 className="font-bold text-olive-800 text-lg mb-2">Buscar Sabedoria Divina</h3>
                  <p className="text-olive-700 mb-3">
                    Reserve um tempo para orar sobre suas finanças e buscar sabedoria divina.
                  </p>
            ...
            <div className="group bg-gradient-to-r from-azure-50 to-azure-100 border-2 border-azure-200 rounded-xl p-6 hover:shadow-lg transition-all duration-300">
              <div className="flex items-start">
                <div className="h-10 w-10 rounded-full bg-azure-500 flex items-center justify-center mr-4 shadow-lg group-hover:scale-110 transition-transform duration-300">
                  <span className="text-white text-lg font-bold">
                    {(monthlyNetBalance < 0 && tithePercentage < 10) ? '4' : 
                     (monthlyNetBalance < 0 || tithePercentage < 10) ? '3' : '2'}
                  </span>
                </div>
                <div className="flex-1">
                  <h3 className="font-bold text-azure-800 text-lg mb-2">Revisar Progresso das Metas</h3>
                  <p className="text-azure-700 mb-3">
                    Avalie o progresso de suas metas e faça ajustes conforme necessário.
                  </p>
