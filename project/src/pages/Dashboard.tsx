...
const Dashboard: React.FC = () => {
  ...
  const currentMonthTotalExpenses = currentMonthTransactionExpenses + currentMonthDedicatedExpenses;
  ...
  const currentMonthTithePercentage = currentMonthIncome > 0 
  ...
  }, [expenses, transactions, tithingRecords, getTotalIncome, getTotalExpenses, getTotalGiving]);
  
  // Current month balance
  const currentMonthBalance = currentMonthIncome - currentMonthTotalExpenses - currentMonthTithingRecords;
  ...
  expenseCategoriesFromExpenses.forEach(expenseCategory => {
    const existingCategory = combinedExpenseCategories.find(item => item.name === expenseCategory.name);
    if (existingCategory) {
  ...
  const getMonthlyTrend = (months: number) => {
    ...
    for (let i = months - 1; i >= 0; i--) {
      ...
      const totalMonthlyExpenses = monthlyTransactionExpenses + monthlyDedicatedExpenses;
      const monthlySaldo = monthlyIncome - totalMonthlyExpenses - monthlyTithing;
  ...
  const COLORS = ['#8fa84b', '#3889cc', '#f5c935', '#e77c64', '#9f86c0', '#f97316'];
  ...
  const financialSummaryLegendData = financialSummary.map((item) => ({
    ...
    percentage: financialSummary.reduce((sum, i) => sum + i.value, 0) > 0 
      ? ((item.value / financialSummary.reduce((sum, i) => sum + i.value, 0)) * 100).toFixed(1) 
      : '0.0'
  }));

  // Expense status summary - rounded values (current month only)
  const paidExpenses = Math.round(expenses.filter(e => e.date.startsWith(currentMonth) && e.status === 'paid').reduce((sum, e) => sum + e.amount, 0) * 100) / 100;
  const pendingExpenses = Math.round(expenses.filter(e => e.date.startsWith(currentMonth) && e.status === 'pending').reduce((sum, e) => sum + e.amount, 0) * 100) / 100;
  ...
  const formatTooltipValue = (value: any, name: string) => {
    ...
    return [`R$ ${numValue.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, name];
  };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-olive-50 border-l-4 border-olive-400 p-4 rounded-r-lg">
        <div className="flex items-center">
          ...
          <div>
            <h3 className="text-sm font-medium text-olive-800">Dados do Mês Atual</h3>
            <p className="text-sm text-olive-700">
              Mostrando informações de <span className="font-medium">{new Date().toLocaleDateString('pt-BR', { month: 'long', year: 'numeric' })}</span>. 
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
        <div className="card p-4">
          ...
          <p className="text-xl font-semibold text-gray-800">R$ {currentMonthIncome.toLocaleString('pt-BR')}</p>
        </div>
        ...
        <div className="card p-4">
          <div className="flex items-center mb-3">
            ...
            <div className="ml-3 min-w-0 flex-1">
              <h3 className="text-sm font-medium text-gray-600">Despesas Transações</h3>
            </div>
          </div>
          <p className="text-xl font-semibold text-gray-800">R$ {currentMonthTransactionExpenses.toLocaleString('pt-BR')}</p>
        </div>
        ...
        <div className="card p-4">
          <div className="flex items-center mb-3">
            ...
            <div className="ml-3 min-w-0 flex-1">
              <h3 className="text-sm font-medium text-gray-600">Total Despesas</h3>
            </div>
          </div>
          <p className="text-xl font-semibold text-gray-800">R$ {currentMonthTotalExpenses.toLocaleString('pt-BR')}</p>
        </div>
        ...
        <div className="card p-4">
          ...
          <p className="text-xl font-semibold text-gray-800">R$ {monthlyExpensesFromExpenseTab.toLocaleString('pt-BR')}</p>
          <div className="flex items-center mt-2">
            <CheckCircle className="h-3 w-3 text-green-600 mr-1 flex-shrink-0" />
            <span className="text-xs text-green-600">R$ {monthlyPaidExpenses.toLocaleString('pt-BR')} pagas</span>
          </div>
        </div>
        ...
        <div className="card p-4">
          <div className="flex items-center mb-3">
            ...
            <div className="ml-3 min-w-0 flex-1">
              <h3 className="text-sm font-medium text-gray-600">Dízimos do Mês</h3>
            </div>
          </div>
          <p className="text-xl font-semibold text-gray-800">R$ {currentMonthTithingRecords.toLocaleString('pt-BR')}</p>
          <div className="flex items-center mt-2">
            <span className="text-xs text-gold-600">{currentMonthTithePercentage.toFixed(1)}% da receita</span>
          </div>
        </div>
        
        <div className="card p-4">
          <div className="flex items-center mb-3">
            <div className={`h-10 w-10 rounded-full ${generalNetBalance >= 0 ? 'bg-green-100' : 'bg-red-100'} flex items-center justify-center shadow-sm flex-shrink-0`}>
              <BarChart3 className={`h-5 w-5 ${generalNetBalance >= 0 ? 'text-green-600' : 'text-red-600'}`} />
            </div>
            <div className="ml-3 min-w-0 flex-1">
              <h3 className="text-sm font-medium text-gray-600">Saldo Geral</h3>
            </div>
          </div>
          <p className={`text-xl font-semibold ${generalNetBalance >= 0 ? 'text-green-600' : 'text-red-600'}`}>
      ...
      {currentMonthTithingRecords > 0 && (
        <div className="explanation-banner">
          <div className="flex items-start">
            ...
            <div>
              <h3 className="text-sm font-medium text-gold-800 mb-1">Visão Financeira do Mês Atual</h3>
              <p className="text-sm text-gold-700">
                <span className="font-medium">Receitas do mês: R$ {currentMonthIncome.toLocaleString('pt-BR')}</span>.
                <span className="font-medium"> Despesas do mês: R$ {currentMonthTotalExpenses.toLocaleString('pt-BR')}</span>.
                <span className="font-medium"> Dízimos do mês: R$ {currentMonthTithingRecords.toLocaleString('pt-BR')} ({currentMonthTithePercentage.toFixed(1)}% da receita)</span>.
                <span className="font-medium"> Saldo do mês: R$ {currentMonthBalance.toLocaleString('pt-BR')}</span>.
                <span className="font-medium"> Saldo geral: R$ {generalNetBalance.toLocaleString('pt-BR')}</span>
              </p>
            </div>
          </div>
        </div>
      ...
      {pendingExpenses > 0 && (
        <div className="bg-orange-50 border-l-4 border-orange-400 p-4 rounded-r-lg">
          <div className="flex items-center">
            ...
            <div>
              <h3 className="text-sm font-medium text-orange-800">Despesas Pendentes do Mês</h3>
              <p className="text-sm text-orange-700">
                Você tem <span className="font-medium">R$ {pendingExpenses.toLocaleString('pt-BR')}</span> em despesas pendentes de pagamento neste mês.
                <Link to="/expenses" className="ml-2 text-orange-800 underline hover:text-orange-900">
                  Ver detalhes →
                </Link>
              </p>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="chart-container">
          <h2 className="section-title mb-4">Despesas por Categoria (Mês Atual)</h2>
          <div className="h-72">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="card p-4">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/transactions" className="text-olive-600 hover:text-olive-700 text-sm font-medium">
              Ver todas
            </Link>
          </div>
          ...
          <div className="space-y-3">
            {recentTransactions.map((transaction) => (
              <div key={transaction.id} className="flex items-start justify-between p-3 bg-gray-50 rounded-lg gap-3">
                <div className="min-w-0 flex-1">
                  <p className="font-medium text-gray-800 break-words">{transaction.description}</p>
                ...
                <span className={`font-semibold flex-shrink-0 text-right ${transaction.type === 'income' ? 'text-green-600' : 'text-red-600'}`}>
        ...
        <div className="card p-4">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/expenses" className="text-olive-600 hover:text-olive-700 text-sm font-medium">
              Ver todas
            </Link>
          </div>
          ...
          <div className="space-y-3">
            {recentExpenses.map((expense) => (
              <div key={expense.id} className="flex items-start justify-between p-3 bg-gray-50 rounded-lg gap-3">
                <div className="min-w-0 flex-1">
                  <p className="font-medium text-gray-800 break-words">{expense.name}</p>
                  <div className="flex flex-wrap items-center gap-1 mt-1">
                    <span className="text-sm text-gray-500">{expense.category}</span>
                    <span className="text-sm text-gray-500">•</span>
                    <span className="text-sm text-gray-500">{expense.date.split('-').reverse().join('/')}</span>
                    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${
                      expense.status === 'paid' 
      ...
      <div className="card p-4">
        <div className="flex items-center justify-between mb-4">
          ...
          <Link to="/goals" className="text-olive-600 hover:text-olive-700 text-sm font-medium">
            Ver todas
          </Link>
        </div>
          ...
          {goals.slice(0, 2).map((goal) => (
            <div key={goal.id} className="goal-card">
              <div className="flex justify-between items-start mb-2 gap-3">
                <div className="min-w-0 flex-1">
                  <h3 className="font-medium text-gray-800 break-words">{goal.title}</h3>
